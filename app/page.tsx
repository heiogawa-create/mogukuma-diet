    ...
    <main className="min-h-screen bg-app px-4 py-5 text-cocoa">
      <div className="mx-auto flex min-h-[calc(100vh-40px)] w-full max-w-[430px] flex-col overflow-hidden rounded-[42px] border border-white/70 bg-white/45 shadow-soft backdrop-blur">
        ...
        <section className="flex-1 overflow-y-auto px-5 pb-28">
          ...
          {screen === "meal" && (
            <div className="space-y-5">
              {analyzeResult ? <MoguKuma compact message={analyzeResult.comment} /> : <MoguKuma compact message="写真を撮ったらもぐクマが栄養素を推定するよ！手入力でも記録できるよ🐻" />}

              <label className="block rounded-[36px] border-2 border-dashed border-berry/30 bg-white/80 p-6 text-center shadow-soft cursor-pointer">
                <input ref={fileInputRef} type="file" accept="image/*" capture="environment" className="sr-only" onChange={handlePhotoChange} />
                {photoPreview ? (
                  <img src={photoPreview} alt="食事写真" className="mx-auto h-48 w-full rounded-[24px] object-cover shadow-soft" />
                ) : (
                  <div className="mx-auto flex h-36 items-center justify-center rounded-[30px] bg-gradient-to-br from-sakura to-cream text-6xl shadow-inner">🍱</div>
                )}
                <p className="mt-4 text-lg font-black">{analyzing ? "🔍 もぐクマが解析中..." : "📷 写真を撮る・選ぶ"}</p>
